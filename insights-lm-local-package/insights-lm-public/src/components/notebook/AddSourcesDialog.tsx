...
const AddSourcesDialog = ({
  ...
  const handleFileUpload = async (files: File[]) => {
    if (!notebookId) {
      toast({
        title: "Fehler",
        description: "Keine Arbeitsmappe ausgewählt",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `${files.length} Datei${files.length > 1 ? 'en' : ''} hinzugefügt und Verarbeitung gestartet`
      });
      ...
      toast({
        title: "Fehler",
        description: "Dateien konnten nicht hinzugefügt werden. Bitte versuche es erneut.",
        variant: "destructive"
      });
    }
  };
  ...
  const handleTextSubmit = async (title: string, content: string) => {
    ...
    try {
      ...
      toast({
        title: "Erfolg",
        description: "Text wurde hinzugefügt und zur Verarbeitung gesendet"
      });
    } catch (error) {
      console.error('Error adding text source:', error);
      toast({
        title: "Fehler",
        description: "Textquelle konnte nicht hinzugefügt werden",
        variant: "destructive"
      });
    } finally {
      setIsLocallyProcessing(false);
  ...
  const handleMultipleWebsiteSubmit = async (urls: string[]) => {
    ...
    try {
      ...
      toast({
        title: "Erfolg",
        description: `${urls.length} Website${urls.length > 1 ? 's' : ''} hinzugefügt und zur Verarbeitung gesendet`
      });
      ...
      toast({
        title: "Fehler",
        description: "Websites konnten nicht hinzugefügt werden",
        variant: "destructive"
      });
    } finally {
      setIsLocallyProcessing(false);
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-medium mb-2">Quellen hinzufügen</h2>
              <p className="text-gray-600 text-sm mb-1">Quellen ermöglichen es MetagonLM, seine Antworten auf die Informationen zu stützen, die für dich am wichtigsten sind.</p>
              <p className="text-gray-500 text-xs">
                (Beispiele: Marketingpläne, Kurslektüre, Forschungsnotizen, Besprechungsprotokolle, Verkaufsdokumente, etc.)
              </p>
            </div>
              ...
              <div className="flex flex-col items-center space-y-4">
                ...
                <div>
                  <h3 className="font-medium text-gray-900 mb-2">
                    {isProcessingFiles ? 'Dateien werden verarbeitet...' : 'Quellen hochladen'}
                  </h3>
                  <p className="text-gray-600 text-sm">
                    {isProcessingFiles ? (
                      'Bitte warte, während wir deine Dateien verarbeiten'
                    ) : (
                      <>
                        Ziehe Dateien hierher oder{' '}
                        <button 
                          className="text-[#8f0c41] hover:underline" 
                ...
                <p className="text-xs text-gray-500">
                  Unterstützte Dateitypen: PDF, txt, Markdown, Audio (z.B. mp3)
                </p>
                <input
                  id="file-upload"
                  type="file"
